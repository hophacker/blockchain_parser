...
struct Callback {
	...
	} // Called when a blockchain file is mapped into memory
	virtual void endMap(const uint8_t *p) {
	} // Called when a blockchain file is unmapped from memory
	virtual void startBlock(const uint8_t *p) {
	} // Called when a block is encountered during first pass
	virtual void endBlock(const uint8_t *p) {
	} // Called when an end of block is encountered during first pass
	...
	virtual void endOutput(const uint8_t *p, // Pointer to TX output raw data
	...
	virtual void edge(uint64_t value, // Number of satoshis coming in on this input from upstream transaction
