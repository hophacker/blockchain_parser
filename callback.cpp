...
typedef std::map<uintptr_t, Callback*> CBMap_jie;
...
Callback *Callback::find(const char *name, bool printList) {
	CBMap_jie found;
	...
	printf(
			"      \"parser help\" for a short summary of available commands.\n\n");
	exit(-1);
}
...
void Callback::showAllHelps(bool longHelp) {
	auto e = callbacks->end();
	auto i = callbacks->begin();
	std::sort(i, e, Cmp());
	...
	while (i != e) {
		...
		if (parser) {
			printf(
					"%s\n",
					longHelp ? parser->get_usage().c_str()
