...
typedef GoogMap<Hash256, const uint8_t*, Hash256Hasher, Hash256Equal>::Map
		TXMap;
...
#define DO(x) x
static inline void startBlock(const uint8_t *p) {
	DO(gCallback->startBlock(p));
}
static inline void endBlock(const uint8_t *p) {
	DO(gCallback->endBlock(p));
}
static inline void startTX(const uint8_t *p, const uint8_t *hash) {
	DO(gCallback->startTX(p, hash));
}
static inline void endTX(const uint8_t *p) {
	DO(gCallback->endTX(p));
}
static inline void startInputs(const uint8_t *p) {
	DO(gCallback->startInputs(p));
}
static inline void endInputs(const uint8_t *p) {
	DO(gCallback->endInputs(p));
}
static inline void startInput(const uint8_t *p) {
	DO(gCallback->startInput(p));
}
static inline void endInput(const uint8_t *p) {
	DO(gCallback->endInput(p));
}
static inline void startOutputs(const uint8_t *p) {
	DO(gCallback->startOutputs(p));
}
static inline void endOutputs(const uint8_t *p) {
	DO(gCallback->endOutputs(p));
}
static inline void startOutput(const uint8_t *p) {
	DO(gCallback->startOutput(p));
}
static inline void start(const Block *s, const Block *e) {
	DO(gCallback->start(s, e));
}
#undef DO

static inline void startMap(const uint8_t *p) {
	gCallback->startMap(p);
}
static inline void endMap(const uint8_t *p) {
	gCallback->endMap(p);
}
static inline void startBlock(const Block *b) {
	gCallback->startBlock(b, gChainSize);
}
static inline void endBlock(const Block *b) {
	gCallback->endBlock(b);
}

static inline void endOutput(const uint8_t *p, uint64_t value,
		const uint8_t *txHash, uint64_t outputIndex,
		const uint8_t *outputScript, uint64_t outputScriptSize) {
	gCallback->endOutput(p, value, txHash, outputIndex, outputScript,
			outputScriptSize);
}

static inline void edge(uint64_t value, const uint8_t *upTXHash,
	...
	gCallback->edge(value, upTXHash, outputIndex, outputScript,
			outputScriptSize, downTXHash, inputIndex, inputScript,
			inputScriptSize);
}

template<bool skip, bool fullContext>
static void parseOutput(const uint8_t *&p, const uint8_t *txHash,
		uint64_t outputIndex, const uint8_t *downTXHash,
		uint64_t downInputIndex, const uint8_t *downInputScript,
		uint64_t downInputScriptSize, bool found = false) {
	if (!skip && !fullContext)
		startOutput(p);
	...
	if (!skip && fullContext && found) {
		edge(value, txHash, outputIndex, outputScript, outputScriptSize,
				downTXHash, downInputIndex, downInputScript,
				downInputScriptSize);
	}
...
template<bool skip, bool fullContext>
static void parseOutputs(const uint8_t *&p, const uint8_t *txHash,
		uint64_t stopAtIndex = -1, const uint8_t *downTXHash = 0,
		uint64_t downInputIndex = 0, const uint8_t *downInputScript = 0,
	...
	for (uint64_t outputIndex = 0; outputIndex < nbOutputs; ++outputIndex) {
		...
		parseOutput<skip, fullContext> (p, txHash, outputIndex, downTXHash,
				downInputIndex, downInputScript, downInputScriptSize, found);
		if (found)
			break;
	}
...
template<bool skip>
static void parseInput(const uint8_t *&p, const uint8_t *txHash,
		uint64_t inputIndex) {
	if (!skip)
		startInput(p);
	...
	if (!skip && 0 != upTXOutputs) {
		...
		parseOutputs<false, true> (upTXOutputs, upTXHash, upOutputIndex,
				txHash, inputIndex, inputScript, inputScriptSize);
	}
...
template<bool skip>
static void parseInputs(const uint8_t *&p, const uint8_t *txHash) {
	if (!skip)
		startInputs(p);
...
template<bool skip>
static void parseTX(const uint8_t *&p) {
	uint8_t *txHash = 0;
...
static void mapBlockChainFiles() {
	...
	std::string blockDir = homeDir + coinName + std::string("blocks_orphan");
	...
	int blkDatId = std::stoi(mapArgs["-startBlk"]);
	int endBlkDatId = std::stoi(mapArgs["-endBlk"]);
	const char *fmt = oldStyle ? "blk%04d.dat" : "blocks/blk%05d.dat";
	printf("what's wrong begin!\n"); //jie
	while (1) {
		if (blkDatId == endBlkDatId + 1)
			break;
		char buf[64];
		sprintf(buf, fmt, blkDatId++);

		std::string blockMapFileName = homeDir + coinName + std::string(buf);

		int blockMapFD = open(blockMapFileName.c_str(), O_DIRECT | O_RDONLY);
		if (blockMapFD < 0) {
			if (1 < blkDatId)
				break;
			sysErrFatal("failed to open block chain file %s",
		...
		if (r < 0)
			sysErrFatal("failed to fstat block chain file %s",
					blockMapFileName.c_str());

		size_t mapSize = statBuf.st_size;
		void *pMap = mmap(0, mapSize, PROT_READ, MAP_PRIVATE, blockMapFD, 0);
		if (((void*) -1) == pMap) {
			sysErrFatal("failed to mmap block chain file %s",
					blockMapFileName.c_str());
		}
...
static void linkBlock(Block *block) {
...
static void linkAllBlocks() {
	...
	printf("linkAllBlocks started!\n"); //jie
	while (i != e) {

		Block *block = (i++)->second;
		linkBlock(block);
	}
	printf("printblocktree started!\n"); //jie
	i = gBlockMap.begin();
	std::ofstream pout("parser.output");
	while (i != e) {
		//showHex((const uint8_t *)i->first, 32, false);
		showHash256(pout, i->second);
		pout << " ";

		Block *block = i->second;

		pout << " ";
		showHash256(pout, block->prev);
		//printf(" ");
		//showHash256(block->next);
		pout << " ";
		pout << block->height << std::endl;
		i++;

	}
	printf("printblocktree ended!\n"); //jie

}

static bool buildBlock(const uint8_t *&p, const uint8_t *e) {
...
static void firstPass() {
	printf("firstPass started!\n"); //jie
	buildNullBlock();
	buildAllBlocks();
	linkAllBlocks();
	printf("firstPass ended!\n"); //jie
}
...
static void cleanMaps() {
	...
	while (i != e) {
		...
		if (r < 0)
			sysErr("failed to unmap block chain file %s", map.name.c_str());
...
int main(int argc, char *argv[]) {
	...
	std::cout << mapArgs["-startBlk"] << std::endl;
	std::cout << mapArgs["-endBlk"] << std::endl;
