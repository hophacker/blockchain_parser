...
static string str_join_trans(const string& sep, InputIterator begin,
		InputIterator end, UnaryOperator op) {
	string buf;
...
static string str_join(const string& sep, InputIterator begin,
		InputIterator end) {
...
static string str_replace(const string& s, const string& patt,
		const string& repl) {
	string tmp = s;
	str_replace(tmp, patt, repl);
	return tmp;
}
static string str_format(const string& s, size_t pre, size_t len,
...
OptionParser& OptionParser::add_option_group(const OptionGroup& group) {
	for (list<Option>::const_iterator oit = group._opts.begin();
			oit != group._opts.end(); ++oit) {
...
const Option& OptionParser::lookup_long_opt(const string& opt) const {
	...
	for (optMap::const_iterator it = _optmap_l.begin(); it != _optmap_l.end();
			++it) {
...
Values& OptionParser::parse_args(const int argc,
...
Values& OptionParser::parse_args(const vector<string>& v) {
	...
	if (add_version_option() and version() != "") {
		add_option("--version").action("version").help(
				_("show program's version number and exit"));
		_opts.splice(_opts.begin(), _opts, --(_opts.end()));
	}
	if (add_help_option()) {
		add_option("-h", "--help").action("help").help(
	...
	for (strMap::const_iterator it = _defaults.begin(); it != _defaults.end();
			++it) {
		if (not _values.is_set(it->first))
			_values[it->first] = it->second;
	}

	for (list<Option>::const_iterator it = _opts.begin(); it != _opts.end();
			++it) {
...
void OptionParser::process_opt(const Option& o, const string& opt,
		const string& value) {
	if (o.action() == "store") {
...
string OptionParser::format_help(unsigned int indent /* = 4 */) const {
	...
	if (!_opts.empty()) {
		...
		for (list<OptionGroup const*>::const_iterator it = _groups.begin();
				it != _groups.end(); ++it) {
			const OptionGroup& group = **it;
			ss << endl << pad << pad << "  " << group.title() << ":" << endl;
			if (group.group_description() != "")
				ss << str_format(group.group_description(), 2 * indent, cols())
						<< endl;
...
string Option::check_type(const string& opt, const string& val) const {
	...
	if (type() == "int" || type() == "long") {
		...
		if (not (ss >> t))
			err << _("option") << " " << opt << ": "
					<< _("invalid integer value") << ": '" << val << "'";
	} else if (type() == "float" || type() == "double") {
		double t;
		if (not (ss >> t))
			err << _("option") << " " << opt << ": "
					<< _("invalid floating-point value") << ": '" << val << "'";
	} else if (type() == "choice") {
		if (find(choices().begin(), choices().end(), val) == choices().end()) {
			list<string> tmp = choices();
			transform(tmp.begin(), tmp.end(), tmp.begin(), str_wrap("'"));
			err << _("option") << " " << opt << ": " << _("invalid choice")
					<< ": '" << val << "'" << " (" << _("choose from") << " "
					<< str_join(", ", tmp.begin(), tmp.end()) << ")";
		}
	} else if (type() == "complex") {
		complex<double> t;
		if (not (ss >> t))
			err << _("option") << " " << opt << ": "
					<< _("invalid complex value") << ": '" << val << "'";
	}
...
string Option::format_option_help(unsigned int indent /* = 2 */) const {
	...
	if (not _short_opts.empty()) {
		ss
				<< str_join_trans(", ", _short_opts.begin(), _short_opts.end(),
						str_wrap("-", mvar_short));
	...
	if (not _long_opts.empty())
		ss
				<< str_join_trans(", ", _long_opts.begin(), _long_opts.end(),
						str_wrap("--", mvar_long));
...
string Option::format_help(unsigned int indent /* = 2 */) const {
	...
	if (help() != "") {
		string help_str =
				(get_default() != "") ?
						str_replace(help(), "%default", get_default()) : help();
